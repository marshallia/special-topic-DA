...
MODEL_SAVE_PATH = 'model/dann_resnet18.pth'
...
class DANNModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.feature.fc = nn.Identity()  # Remove classifier
    ...
    def forward(self, x, alpha=0):
        ...
        reverse_feat = grad_reverse(feat, alpha)
        domain_output = self.domain_classifier(reverse_feat)
        return class_output, domain_output

model = DANNModel(NUM_CLASSES).to(DEVICE)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    # Validation on real labeled val set
    model.eval()
    val_correct = 0
    with torch.no_grad():
    ...
    if val_acc > best_acc:
        ...
        print(f"Saved best DANN model with Val Acc: {best_acc:.4f}")
